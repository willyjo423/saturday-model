...
BACKTEST_PATH = config.DATA / "backtest.parquet"


def main(argv=None) -> int:
    p = argparse.ArgumentParser(description="Train CFB prediction models")
    p.add_argument("--data", default=str(config.DATA / "training.parquet"))
    ...
    feat = pd.read_parquet(args.data)
    ...
    if not args.skip_backtest:
        ...
        if not oos.empty:
            oos.to_parquet(BACKTEST_PATH, index=False)
